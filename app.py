...
if raw_df is not None:
    ...
    # TAB 2: SIMULATOR (THE LINKEDIN SHOWCASE)
    with tab2:
        if 'model' not in st.session_state:
            ...
            # Layout: Inputs on Left, Big Numbers on Right
            ...
            with sim_col2:
                ...
                input_row = st.session_state['last_row'].to_dict() # Default values
                input_row.update({
                    'date': d_val,
                    'food_name': sel_food,
                ...
                # Ensure input df has correct columns
                pred_df = pd.DataFrame([input_row])
                for f in feats:
                    if f not in pred_df.columns: pred_df[f] = 0 # Safety fill
                ...
                with c_a:
                    st.metric("Proj. Revenue", f"₹{revenue:,}")
                with c_b:
                    delta = prediction - 100 # Dummy baseline
    ...
    with tab3:
        ...
        fig = px.area(trend_df, x='date', y='actual_units_sold', 
                      title="Global Demand Trend", 
